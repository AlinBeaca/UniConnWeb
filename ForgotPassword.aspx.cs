...
namespace UniConnWeb
{
    public partial class ForgotPassword : System.Web.UI.Page
    {
        ...
        protected void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            OracleDataAdapter sda = new OracleDataAdapter("select Utilizator from Userul WHERE Utilizator='" + txtemail.Text + "'", conn);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    conn.Open();
                    OracleCommand cmd = new OracleCommand("update Userul set Parola ='" + txtconpass.Text + "' where Utilizator = '" + txtemail.Text + "'", conn);
                    cmd.ExecuteNonQuery();
